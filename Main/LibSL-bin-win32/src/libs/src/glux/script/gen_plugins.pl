...
sub write_h_typedefs()
{
    ...
    {
		$fct=shift @fcts;
		print $OUT "#ifndef __GLUX__GLFCT_".$fct."\n";
		my $decl=$$typ[1];
...
sub write_cpp_globals()
{
    ...
    foreach my $typ (@$typstbl)
    {
		$fct=shift @fcts;
		print $OUT "#ifndef __GLUX__GLFCT_".$fct."\n";
      	print $OUT $$typ[0]." "."glux::".$fct."=glux_fake_".$fct.";\n";
		print $OUT "#endif\n";
    }    
}
...
sub write_cpp_fake()
{
    ...
    foreach my $typ (@$typstbl)
    {
		my $fct=shift @fcts;
		my $proto=$$typ[1];
		$proto=~s/\(.*?\)/APIENTRY glux_fake_$fct/;
		$proto=~s/typedef//;
		$proto=~s/;//;
		print $OUT "#ifndef __GLUX__GLFCT_".$fct."\n";
		print $OUT $proto;
...
#	print $OUT "  std::cerr << \"FATAL: Extension $ext was not loaded or is not fully available.\" << std::endl << \"=> cannot use $fct.\" << std::endl;\n";
#	print $OUT "# endif\n";
# 	print $OUT "  exit (-1);\n";
		print $OUT "# ifndef GLUX_NO_OUTPUT\n";
		print $OUT "  std::cerr << \"gluX - loading extension $ext (triggered by $fct)\" << std::endl;\n";
		print $OUT "# endif\n";
...
sub write_cpp_load()
{
	...
	my $fct=shift @fcts;
	print $OUT "
#ifndef __GLUX__GLFCT_".$fct."
     glux::$fct = ($$typ[0])GLUX_LOAD_PROC(\"$fct\");
     if(NULL == glux::$fct)
     {
        glux::$fct=glux_fake_$fct;
        if (!m_bDevel)
	   return false;
        else
        {\n";
