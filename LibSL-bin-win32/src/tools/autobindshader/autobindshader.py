...
print(' [AutoBindShader] - v0.2 - ' + now.strftime("%Y-%m-%d %H:%M") + ' <' + shader + '>  ');
...
def appendFileToString( fname, notweak ) :
		...
		for l in lines:
				...
				if matched_if != None:
					...
					# restart string
					fcpp.write(' std::string("\\\n')
					# remeber a 'else' was encountered
...
fcpp.write('		GLShader::t_GeometryShaderNfo gsCode() {')
fcpp.write('		  GLShader::t_GeometryShaderNfo gs;')
...
# init parameters
fcpp.write('void initParameters(GL{0}& shader,bool firstInit) {{\n'.format(className));
...
# access to uniforms
for v in allUniforms:
	fcpp.write('		GLParameter ' + v.group(2) + '; /// ' + v.group(1) + '\n' )
# access to defines
for v in allDefines.keys():
	fcpp.write('		bool		' + v + ';\n' )
